-- Clean setup script that handles existing data
-- This will safely reset and recreate everything

-- First, let's see what exists
DO $$
BEGIN
    RAISE NOTICE 'Checking existing data...';
    
    -- Check if pokemon_cards table exists
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'pokemon_cards') THEN
        RAISE NOTICE 'pokemon_cards table exists';
    ELSE
        RAISE NOTICE 'pokemon_cards table does not exist';
    END IF;
    
    -- Check if user exists
    IF EXISTS (SELECT FROM auth.users WHERE email = 'owner@pokemoncards.com') THEN
        RAISE NOTICE 'User owner@pokemoncards.com already exists';
    ELSE
        RAISE NOTICE 'User owner@pokemoncards.com does not exist';
    END IF;
END $$;

-- Clean up existing data (optional - uncomment if you want to start fresh)
-- DROP TABLE IF EXISTS pokemon_cards CASCADE;
-- DELETE FROM auth.identities WHERE user_id IN (SELECT id FROM auth.users WHERE email = 'owner@pokemoncards.com');
-- DELETE FROM auth.users WHERE email = 'owner@pokemoncards.com';

-- Create pokemon_cards table (with IF NOT EXISTS)
CREATE TABLE IF NOT EXISTS pokemon_cards (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  category TEXT NOT NULL DEFAULT 'Trading Card Games',
  sub_category TEXT NOT NULL DEFAULT 'Pokémon Cards',
  title TEXT NOT NULL,
  description TEXT,
  quantity INTEGER NOT NULL DEFAULT 0,
  type TEXT NOT NULL,
  price DECIMAL(10,2) NOT NULL DEFAULT 0.00,
  shipping_profile TEXT,
  offerable BOOLEAN DEFAULT TRUE,
  hazmat BOOLEAN DEFAULT FALSE,
  condition TEXT NOT NULL DEFAULT 'Near Mint',
  cost_per_item DECIMAL(10,2) DEFAULT 0.00,
  sku TEXT UNIQUE,
  image_url_1 TEXT,
  image_url_2 TEXT,
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- Create indexes (with IF NOT EXISTS)
CREATE INDEX IF NOT EXISTS idx_pokemon_cards_user_id ON pokemon_cards(user_id);
CREATE INDEX IF NOT EXISTS idx_pokemon_cards_sku ON pokemon_cards(sku);
CREATE INDEX IF NOT EXISTS idx_pokemon_cards_category ON pokemon_cards(category, sub_category);

-- Enable RLS
ALTER TABLE pokemon_cards ENABLE ROW LEVEL SECURITY;

-- Drop existing policies if they exist
DROP POLICY IF EXISTS "Users can view their own cards" ON pokemon_cards;
DROP POLICY IF EXISTS "Users can insert their own cards" ON pokemon_cards;
DROP POLICY IF EXISTS "Users can update their own cards" ON pokemon_cards;
DROP POLICY IF EXISTS "Users can delete their own cards" ON pokemon_cards;

-- Create fresh policies
CREATE POLICY "Users can view their own cards" ON pokemon_cards
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can insert their own cards" ON pokemon_cards
  FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their own cards" ON pokemon_cards
  FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Users can delete their own cards" ON pokemon_cards
  FOR DELETE USING (auth.uid() = user_id);

-- Handle user creation safely
DO $$
DECLARE
    user_id uuid;
    existing_user_id uuid;
BEGIN
    -- Check if user already exists
    SELECT id INTO existing_user_id FROM auth.users WHERE email = 'owner@pokemoncards.com';
    
    IF existing_user_id IS NOT NULL THEN
        RAISE NOTICE 'User already exists with ID: %', existing_user_id;
        
        -- Update the existing user's password
        UPDATE auth.users 
        SET 
            encrypted_password = crypt('pokemon123', gen_salt('bf')),
            updated_at = now(),
            email_confirmed_at = COALESCE(email_confirmed_at, now())
        WHERE email = 'owner@pokemoncards.com';
        
        user_id := existing_user_id;
        RAISE NOTICE 'Updated existing user password';
    ELSE
        -- Create new user
        user_id := gen_random_uuid();
        
        INSERT INTO auth.users (
            id,
            instance_id,
            aud,
            role,
            email,
            encrypted_password,
            email_confirmed_at,
            created_at,
            updated_at,
            raw_app_meta_data,
            raw_user_meta_data
        ) VALUES (
            user_id,
            '00000000-0000-0000-0000-000000000000',
            'authenticated',
            'authenticated',
            'owner@pokemoncards.com',
            crypt('pokemon123', gen_salt('bf')),
            now(),
            now(),
            now(),
            '{"provider":"email","providers":["email"]}',
            '{"is_owner":true}'
        );
        
        RAISE NOTICE 'Created new user with ID: %', user_id;
    END IF;
    
    -- Handle identity (delete and recreate to avoid conflicts)
    DELETE FROM auth.identities WHERE user_id = user_id AND provider = 'email';
    
    INSERT INTO auth.identities (
        id,
        user_id,
        identity_data,
        provider,
        created_at,
        updated_at
    ) VALUES (
        gen_random_uuid(),
        user_id,
        format('{"sub":"%s","email":"%s"}', user_id, 'owner@pokemoncards.com')::jsonb,
        'email',
        now(),
        now()
    );
    
    RAISE NOTICE 'Identity created/updated for user';
END $$;

-- Final verification
SELECT 
    'User verification:' as check_type,
    id,
    email,
    email_confirmed_at IS NOT NULL as email_confirmed,
    created_at
FROM auth.users 
WHERE email = 'owner@pokemoncards.com';

SELECT 
    'Identity verification:' as check_type,
    i.id,
    i.provider,
    u.email
FROM auth.identities i
JOIN auth.users u ON i.user_id = u.id
WHERE u.email = 'owner@pokemoncards.com';

RAISE NOTICE 'Setup completed successfully!';
